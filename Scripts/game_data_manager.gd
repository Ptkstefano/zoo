...
func build_enclosure(id, coordinates, entrance_cell, fence_res):
	...
	if !current_enclosure:
		...
		print('a')
		var save_data = {"savetype": IdRefs.SAVE_TYPES.NEW_ENCLOSURE, "id": current_enclosure.id, "cells": current_enclosure.enclosure_cells, "entrance_door_cell": null, "res_path": current_enclosure.fence_res.get_path()}
		#SignalBus.save_new_enclosure.emit(save_data)

	##Expand existing enclosure
	else:
		current_enclosure.fence_res = selected_fence
		current_enclosure.add_cells(coordinates, selected_fence)
		var save_data = {"savetype": IdRefs.SAVE_TYPES.UPDATE_ENCLOSURE, "id": current_enclosure.id, "cells": current_enclosure.enclosure_cells, "entrance_door_cell": current_enclosure.entrance_door_cell, "res_path": current_enclosure.fence_res.get_path()}
		#SignalBus.save_update_enclosure.emit(save_data)
	...
	for coordinate in coordinates:
		...
		for enclosure in get_children():
			if enclosure.enclosure_cells.has(coordinate):
				...
				#var save_data = {"savetype": IdRefs.SAVE_TYPES.UPDATE_ENCLOSURE, "id": enclosure.id, "cells": enclosure.enclosure_cells, "entrance_door_cell": enclosure.entrance_door_cell, "res_path": enclosure.fence_res.get_path()}
				#SignalBus.save_update_enclosure.emit(save_data)
